  ...
  <header class="topbar">
    <div class="nav-shell">
      ...
      <div class="nav-actions">
        <router-link to="/account" class="profile-button" title="账户设置">
          <span class="avatar">
    ...
    <transition name="menu">
      <div v-if="mobileOpen" class="mobile-panel">
        <nav>
          ...
          <router-link to="/account" :class="{ active: isActive('/account') }" @click="mobileOpen = false">
...
const profile = reactive({ username: userInfo.username || '', nickname: userInfo.nickname || '', avatar: localStorage.getItem('user_avatar') || userInfo.avatar || '', role: userInfo.role })

const isAdmin = computed(() => profile.role === 1 || userInfo.role === 1 || userInfo.isAdmin === true)
const displayName = computed(() => profile.nickname || profile.username || '用户')
...
@media(max-width:760px){.topbar{padding:.55rem .65rem}.nav-shell{min-height:3.45rem;border-radius:1rem}.desktop-nav,.desktop-logout,.profile-button{display:none}.brand{margin-right:auto}.menu-button{display:grid;place-items:center}.mobile-panel{display:block;width:calc(100% - 1.3rem);margin:.45rem auto 0;padding:.55rem;border:1px solid rgba(212,121,143,.14);border-radius:1rem;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(77,46,56,.14);backdrop-filter:blur(20px)}.dark .mobile-panel{background:rgba(29,34,41,.98);border-color:rgba(255,255,255,.07)}.mobile-panel nav{display:grid;gap:.15rem;margin-bottom:.45rem}.mobile-panel nav a{display:grid;grid-template-columns:1.5rem 1fr auto;align-items:center;gap:.45rem;padding:.68rem;border-radius:.72rem;color:#747d87;font-size:.78rem;font-weight:700}.mobile-panel nav a.active{color:#b64b63;background:#fff1f4}.dark .mobile-panel nav a.active{color:#ff9bb0;background:#3b3037}.mobile-panel .tail{font-size:.8rem;color:#b7afb2}}
</style>
