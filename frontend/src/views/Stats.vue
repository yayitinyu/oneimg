<template>
  <div class="stats-page">
    <PageHeading
      :eyebrow="isAdmin ? 'System pulse' : 'Your image pulse'"
      :title="isAdmin ? '全站统计' : '我的统计'"
      :description="isAdmin ? '从上传趋势到存储分布，快速了解整个图床。' : '只统计你自己上传且仍在生命周期内的图片。'"
    >
      <template #action>
        ...
        <article class="panel trend-panel">
          <div class="panel-heading">
            ...
            <div class="chart-controls">
              ...
              <AppSelect v-model="period" class="period-select" :options="periodOptions" aria-label="选择统计周期" @change="loadPeriod" />
            </div>
          </div>
...
const trendData = ref([])
const periodOptions = [
  { value: 'dashboard', label: '近 14 天' },
  { value: 'day', label: '近 30 天' },
  { value: 'week', label: '近 12 周' },
  { value: 'month', label: '近 12 月' },
  { value: 'year', label: '近 5 年' },
]
const stats = ref({
...
.period-select{width:7.4rem}.period-select :deep(.app-select-trigger){min-height:2.05rem;padding:.35rem .48rem .35rem .58rem;border-radius:.58rem;font-size:.66rem}.period-select :deep(.app-select-menu){min-width:8.5rem}
@media(max-width:640px){.refresh-button{width:auto}}
</style>
